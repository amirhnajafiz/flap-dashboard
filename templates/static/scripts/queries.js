...
function fetch_query() {
    ...
    fetch(`/api/files/${type}?page=${currentPage}&proc=${encodeURIComponent(proc.value)}&hunk=${hunk}&rmstd=${stds}&desc=${ro}`)
        .then(res => res.json())
        .then(render_query)
...
function render_query(resp) {
    ...
    resp.data.forEach(row => {
        ...
        tr.innerHTML = `
            <td style="border: 1px solid black; padding: 4px;">${row.fname}</td>
    ...
    document.getElementById("page_info").textContent =
        `Page ${resp.page} of ${resp.total_pages}`;
