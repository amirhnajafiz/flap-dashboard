  ...
  const xScale = d3
    ...
    .domain([
      d3.min([
        ...
        d3.min(rendom_events, (d) => d.time),
      ]),
      d3.max([
        d3.max(events, (d) => d.end),
        d3.max(rendom_events, (d) => d.time),
      ]),
    ])
    .range([0, innerWidth])
    .nice();
...
function fetch_io_events(proc_name, fname = "") {
  ...
  const url = `/api/events/io?proc=${encodeURIComponent(
    proc_name
  )}&fname=${encodeURIComponent(fname)}&hunk=${hunk}&rmstd=${stds}`;
  fetch(url)
